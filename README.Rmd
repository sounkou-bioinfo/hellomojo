...
and scalar Int/Float types are currently supported. This is quite brittle now.

**Note**: This requires Mojo to be installed via `mojo_install()` first. The installation
downloads ~1GB of dependencies, so it's not run in this README by default.



## Limitations to investigate

The package now offers two approaches: the original pixi-based static compilation and a new dynamic compilation system using Python virtual environments. For static compilation, we use pixi to get Mojo binaries, but installing this on the R Windows toolchain is not straightforward. We should consider using uv or creating an R package that installs pixi. Moreover, Windows support could not be added to the current pixi workspace (because Mojo only supports WSL). This package should work fine on Unix systems.
